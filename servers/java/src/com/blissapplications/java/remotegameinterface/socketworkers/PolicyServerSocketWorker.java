...
public class PolicyServerSocketWorker extends ServerSocketWorker {
	...
	public void clientServed(IClientConnection servedClient) {
		_logger.info(String.format(
				"Client %1$s has been served by policy server.", servedClient));
		synchronized (this) {
			if (_policyServerClientConnections.contains(servedClient)) {
				_policyServerClientConnections.remove(servedClient);
			} else {
				_logger.warn(String
						.format("Client %1$s not found on policy server internal state! Proceeding...",
								servedClient));
			}
		}
	}
	...
	public void clientDisconnected(IClientConnection disconnectedClient) {
		_logger.info(String.format("Client %1$s has disconnected.",
				disconnectedClient));
		synchronized (this) {
			if (_policyServerClientConnections.contains(disconnectedClient)) {
				_policyServerClientConnections.remove(disconnectedClient);
			} else {
				_logger.warn(String
						.format("Client %1$s not found on policy server internal state! Proceeding...",
								disconnectedClient));
